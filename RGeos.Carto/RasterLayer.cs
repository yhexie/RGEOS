...
namespace RGeos.Carto
{
    ...
    /// 通过GDAL读取栅格影像
    /// </summary>
    public class RasterLayer : Layer, IDisposable
    {
        ...
        //进行变换
        internal AffineTransformation2D _geoTransform;
        ...
        /// 初始化栅格图层
        /// </summary>
        /// <param name="strLayerName">layer的名称</param>
        /// <param name="imageFilename">影像文件路径</param>
        public RasterLayer(string strLayerName, string imageFilename)
        {
            Name = strLayerName;
            ...
            try
            {
                ...
                //没有从影像文件中读取到投影信息，查找prj投影文件
                if (_projectionWkt == "" && File.Exists(imageFilename.Substring(0, imageFilename.LastIndexOf(".")) + ".prj"))
                {
                    _projectionWkt = File.ReadAllText(imageFilename.Substring(0, imageFilename.LastIndexOf(".")) + ".prj");
                }

                _imagesize = new Size(_gdalDataset.RasterXSize, _gdalDataset.RasterYSize);
                // Debug.WriteLine(string.Format("{0},{1}", _gdalDataset.RasterXSize, _gdalDataset.RasterYSize));
                //读取影像范围
                _envelope = GetExtent();
        ...
        public override void Draw(IScreenDisplay display)
        {
            ...
            Rectangle rect = display.DisplayTransformation.DeviceFrame;
            Graphics g = display.BitMapGc;
            if (g != null)
            {
                GetPreview(g, _gdalDataset, rect, display.DisplayTransformation.VisibleBounds, null, display);
            }
        }
        ...
        public double GetOneToOne(IScreenDisplay map)
        {
            ...
            BoundingBox bbox = map.DisplayTransformation.VisibleBounds;
            Rectangle size = map.DisplayTransformation.DeviceFrame;
            ...
            if ((dblImginMapW > dblImginMapH && DsWidth > DsHeight) ||
                ...
                return map.DisplayTransformation.Zoom * (dblImginMapW / DsWidth);
            // image was turned on its side
            else
                return map.DisplayTransformation.Zoom * (dblImginMapH / DsWidth);
        }

        // zooms to nearest tiff internal resolution set
        public double GetZoomNearestRSet(IScreenDisplay map, bool bZoomIn)
        {
            double DsWidth = _imagesize.Width;
            double DsHeight = _imagesize.Height;
            double left, top, right, bottom;
                    ...
                    for (int i = 0; i <= 8; i++)
                    {
                        if (DsWidth / Math.Pow(2, i) > dblTempWidth)
                        {
                            if (DsWidth / Math.Pow(2, i + 1) < dblTempWidth)
                                return map.DisplayTransformation.Zoom * (dblTempWidth / (DsWidth / Math.Pow(2, i)));
                        }
                    }
                }
                else
            ...
            return map.DisplayTransformation.Zoom;
        }
        ...
        private BoundingBox GetExtent()
        {
            if (_gdalDataset != null)
            {
                ...
                _geoTransform = new AffineTransformation2D(geoTrans);
        ...
        public Collection<RGeos.Geometries.RgPoint> GetFourCorners()
        {
            Collection<RgPoint> points = new Collection<RgPoint>();
            double[] dblPoint;
            ...
            if (_gdalDataset != null)
            {
                ...
                points.Add(new RgPoint(geoTrans[0], geoTrans[3]));
                points.Add(new RgPoint(geoTrans[0] + (geoTrans[1] * _imagesize.Width),
                                                geoTrans[3] + (geoTrans[4] * _imagesize.Width)));
                points.Add(
                    new RgPoint(geoTrans[0] + (geoTrans[1] * _imagesize.Width) + (geoTrans[2] * _imagesize.Height),
                                         geoTrans[3] + (geoTrans[4] * _imagesize.Width) + (geoTrans[5] * _imagesize.Height)));
                points.Add(new RgPoint(geoTrans[0] + (geoTrans[2] * _imagesize.Height),
                ...
                if (_transform != null)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        points[i] = new RgPoint(dblPoint[0], dblPoint[1]);
                    }
                }
            }
        ...
        /// 获取影像的预览图
        /// </summary>
        /// <param name="grph">Graphics</param>
        /// <param name="dataset">GDAl读取的栅格数据集</param>
        /// <param name="size">地图的屏幕大小</param>
        /// <param name="displayBbox">控件显示范围对应的地图大小</param>
        /// <param name="mapProjection"></param>
        /// <param name="screenDisplay"></param>
        protected virtual void GetPreview(Graphics grph, Dataset dataset, Rectangle size,
                                          BoundingBox displayBbox, ICoordinateSystem mapProjection, IScreenDisplay screenDisplay)
        {
            double[] geoTrans = new double[6];
            _gdalDataset.GetGeoTransform(geoTrans);
            ...
            {
                GetNonRotatedPreview(dataset, size, grph, displayBbox, mapProjection);
                return;
            }
            ...
            RgPoint imageTL = new RgPoint(), imageBR = new RgPoint();
            ...
            if (dataset != null)
            {
                ...
                // bounds of section of image to be displayed,影像和显示的视图范围比较，找出显示的范围
                ...
                Debug.WriteLine(string.Format("{0},{1}", right, bottom));
                ...
                RgPoint groundBR = _geoTransform.ImageToGround(imageBR);
                RgPoint groundTL = _geoTransform.ImageToGround(imageTL);

                // convert ground coordinates to map coordinates to figure out where to place the bitmap
                bitmapBR = new Point((int)screenDisplay.DisplayTransformation.ToScreen(trueImageBbox.BottomRight).X + 1,
                                     (int)screenDisplay.DisplayTransformation.ToScreen(trueImageBbox.BottomRight).Y + 1);
                bitmapTL = new Point((int)screenDisplay.DisplayTransformation.ToScreen(trueImageBbox.TopLeft).X,
                                     (int)screenDisplay.DisplayTransformation.ToScreen(trueImageBbox.TopLeft).Y);
                Debug.WriteLine(string.Format("{0},{1}", bitmapTL.X, bitmapTL.Y));
                ...
                try
                {
                    unsafe
                    {
                        ...
                        double dblMapPixelWidth = screenDisplay.DisplayTransformation.PixelWidth;
                        double dblMapPixelHeight = screenDisplay.DisplayTransformation.PixelHeight;
                        double dblMapMinX = screenDisplay.DisplayTransformation.VisibleBounds.Min.X;
                        double dblMapMaxY = screenDisplay.DisplayTransformation.VisibleBounds.Max.Y;
        ...
        private void GetNonRotatedPreview(Dataset dataset, Rectangle size, Graphics g,
